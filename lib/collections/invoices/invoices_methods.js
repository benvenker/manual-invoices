...
Meteor.methods({
  ciuExport: function(invoice) {
    ...
    var exportDir = '/mnt/ManualEntry/TEST/ben_ciu_test.dat1';
    ...
    */
    ...
    function prependZeros(maxLen, num, recordName) {
      ...
      fs.writeFileSync(exportDir, ciu_mcr + ciu_hdr, function(err) {
        if (err) throw err;
      ...
      for (k = 0; k < invoiceLines.length; k++) {
        ...
        fs.appendFileSync(exportDir, ciu_ddr, function(err) {
          if (err) throw err;
          console.log('ddr\'s appended');
        });
        console.log("i made it to the end when k = " + k);
      }
      fs.appendFileSync(exportDir, 'EOF', function(err) {
        if (err) throw err;
